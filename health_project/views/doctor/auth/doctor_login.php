<?php
  ...
  if(isset($_POST['submit'])){
    ...
    if(!empty($_POST['email']) && !empty($_POST['password'])){
      ...
      $check = mysqli_query($conn, "SELECT * FROM doctor_info WHERE email='$email' AND password='$password' AND user_status='true'");
      $row = mysqli_fetch_array($check);
      if($row['email'] == $email && $row['password'] == $password) {
        $_SESSION['userId'] = $row['id'];
        header('Location: ../../patient_message_requests.php');
        ob_flush();
      } else {
        $error = true;
      }
